...
class OpenUrlPostHttpResponseTest: PostWebConnection.PostWebRequestListener {
    ...
    @Test
    fun testHttp() {
        PostWebConnection(okHttpClient, this, url, Headers, Data
        ).sendPostWebRequest()
        val res: String = response.get().trim()
        Assert.assertTrue(res.startsWith(HTML_RESPONSE_START))
        Assert.assertTrue(res.endsWith(HTML_RESPONSE_END))
    }
