...
import org.catrobat.catroid.common.FlavoredConstants.PRIVACY_POLICY_URL

@Category(OutgoingNetworkTest::class)
@RunWith(AndroidJUnit4::class)
class PrivacyPolicyHttpPostResponseTest : PostWebConnection.PostWebRequestListener  {
    ...
    @Test
    fun testHttps() {
        PostWebConnection(okHttpClient, this, PRIVACY_POLICY_URL, Headers, Data
        ).sendPostWebRequest()
        val body: String
        body = try {
            response.get().trim { it <= ' ' }
        ...
        assertTrue(body.startsWith(HTML_RESPONSE_END));
    }
